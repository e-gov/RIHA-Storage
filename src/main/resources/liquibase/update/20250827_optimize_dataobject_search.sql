-- Optimization for data object search performance
...
DROP MATERIALIZED VIEW IF EXISTS riha.data_object_search_materialized;
...
-- These indexes target the common search patterns from the frontend

-- Primary text search index (for the main search field)
-- Use trigram indexes for better performance with partial matches
...
ON riha.data_object_search_materialized(personal_data, short_name, andmeobjekti_nimi_lower);
...
-- Grant permissions
GRANT SELECT ON riha.data_object_search_materialized TO riha;
GRANT EXECUTE ON FUNCTION riha.refresh_data_object_search_materialized() TO riha;

-- Initial refresh of the materialized view
SELECT riha.refresh_data_object_search_materialized();

-- Create a trigger to automatically refresh the materialized view when underlying data changes
-- Note: This is a simplified approach - in production you might want more sophisticated refresh logic

CREATE OR REPLACE FUNCTION riha.auto_refresh_data_object_search()
RETURNS trigger AS $$
BEGIN
    -- Refresh asynchronously to avoid blocking the main transaction
    PERFORM pg_notify('refresh_data_object_search', '');
    RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
-- Create an improved version of the original view that uses the materialized view when possible
