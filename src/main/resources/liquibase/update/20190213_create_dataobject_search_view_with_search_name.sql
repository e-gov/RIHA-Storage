...
create or replace view riha.main_resource_current_version as
  select mr.* from riha.main_resource mr,
...
create or replace view riha.data_object_search_view as
  select raw_data."Infosüsteem"       as infosystem,
         ...
         COALESCE (mr.json_content #>> '{name}', '') || '  ' || COALESCE (mr.json_content #>> '{short_name}' , '') as search_name,
         f.uuid as file_uuid,
         md5(concat("AV", "Vanemobjekt 1", "Andmeobjekti nimi", "DIA", "EIA", "IA", "Kommentaar", "PA", "Vanemobjekt 1")) as id
  ...
  from (
       ...
       from riha.large_object lo,
         ...
         inner join riha.main_resource_current_version mr on rf.main_resource_uuid :: TEXT = mr.json_content #>> '{uuid}'