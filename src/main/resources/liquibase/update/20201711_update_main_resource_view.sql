...
SELECT DISTINCT ON ((main_resource.json_content #>> '{uuid}')) main_resource.main_resource_id,
                   ...
                   case when
                       COALESCE(jsonb_exists_any(main_resource.json_content -> 'topics', array['x-tee alamsüsteem','X-tee alamsüsteem', 'X-TEE ALAMSÜSTEEM', 'asutusesiseseks kasutamiseks', 'standardlahendus-dhs']), false)
