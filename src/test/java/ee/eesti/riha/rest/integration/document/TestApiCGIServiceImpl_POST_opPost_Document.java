...
public class TestApiCGIServiceImpl_POST_opPost_Document<T> {
  ...
  private static String jsonToUseForCreateShorter = TestFinals.JSON_CONTENT_FOR_DOCUMENT_CORRECT_SAMPLE_AS_JSON_STRING_SHORTER;
 ...
 // create test Main_resource
    ...
    jsonObject = JsonHelper.toJsonObject(jsonToUseForCreateShorter);
    jsonObject.addProperty("main_resource_id", main_resourceId);
    jsonToUseForCreateShorter = jsonObject.toString();
  ...
  public void testCreate_whenJsonMissingRequiredParData_thenError() throws Exception {
    ...
    String json = "{\"op\":\"post\", \"path\": \"" + pathToUse + "\", \"token\":\"testToken\"}";
    Response response = serviceUnderTest.postCGI(json);
  ...
  @Test
  public void testCreate_whenMain_resource_id_OR_Data_object_id_missing_thenError() throws Exception {
    // don't add main_resource_id, use from TestFinals instead
    jsonToUseForCreate = TestFinals.JSON_CONTENT_FOR_DOCUMENT_CORRECT_SAMPLE_AS_JSON_STRING;
    String json = "{\"op\":\"post\", \"path\": \"" + pathToUse + "\","
        + "\"data\":" + jsonToUseForCreate + ", \"token\":\"testToken\"}";
    Response response = serviceUnderTest.postCGI(json);

    RihaRestError error = TestHelper.getObjectFromClient((InputStream) response.getEntity(), RihaRestError.class);
    assertNotNull(error);
    assertEquals(ErrorCodes.DOCUMENT_CREATE_HAS_NO_REF, error.getErrcode());
    assertTrue(error.getErrmsg().contains(ErrorCodes.DOCUMENT_CREATE_HAS_NO_REF_MSG));
    assertFalse(error.getErrtrace().isEmpty());
  ...
  public void testCreate_whenJsonInDataIsMissingRequiredPars_thenError() throws Exception {
    ...
    String jsonInData = "{}";
    String json = "{\"op\":\"post\", \"token\":\"testToken\", \"path\": \"" + pathToUse + "\", \"data\":" + jsonInData
        + "}";
    Response response = serviceUnderTest.postCGI(json);
